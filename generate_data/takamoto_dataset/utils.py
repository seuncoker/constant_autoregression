...
def init_multi(xc, numbers=10000, k_tot=8, init_key=2022, num_choise_k=2, if_norm=False):
    ...
    u = amp * jnp.sin(kk[:, :, jnp.newaxis] * xc[jnp.newaxis, jnp.newaxis, :] + phs*selected[:,:,jnp.newaxis]) # each init same centered at zero
    _u = jnp.sum(u, axis=1)
    ...
    # cond, u_sum = vmap(select_A, 0, 0)(carry)
    # sgn = random.choice(key, a=jnp.array([1, -1]), shape=([numbers, 1]))
    # uu = u_sum * sgn  # random flip of signature
    ...
    #print(jnp.std(jnp.mean(_u,axis=-1)))
    #import pdb; pdb.set_trace()
    return _u
