...
def check_files(pde: PDE, modes: dict) -> None:
    ...
    for mode, replace, num_samples, suffix in modes:
        save_name = "data_3/" + "_".join([str(pde), mode])
        if mode == "train":
...
def check_directory() -> None:
    ...
    """
    if os.path.exists(f'data_3'):
        print(f'Data_3 directory exists and will be written to.')
    else:
        os.mkdir(f'data_3')
        print(f'Data_3 directory created.')

    if not os.path.exists(f'data_3/log'):
        os.mkdir(f'data_3/log')
...
def generate_trajectories(pde: PDE,
    ...
    save_name = "data_3/" + "_".join([pde_string, mode])
    if mode == "train":
        save_name = save_name + "_" + str(num_samples)
    if suffix:
        save_name = save_name + '_' + suffix
...
def generate_data(experiment: str,
    ...
    if args.log:
        logfile = f'data_3/log/{experiment}_time{timestring}.csv'
