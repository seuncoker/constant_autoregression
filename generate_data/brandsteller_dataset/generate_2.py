...
from equation import PDE, KdV, KS
...
def check_files(pde: PDE, modes: dict) -> None:
    ...
    for mode, replace, num_samples, suffix in modes:
        save_name = "data_2/" + "_".join([str(pde), mode])
        if mode == "train":
...
def check_directory() -> None:
    ...
    """
    if os.path.exists(f'data_2'):
        print(f'Data_2 directory exists and will be written to.')
    else:
        os.mkdir(f'data_2')
        print(f'Data_2 directory created.')

    if not os.path.exists(f'data_2/log'):
        os.mkdir(f'data_2/log')
...
def generate_trajectories(pde: PDE,
    ...
    save_name = "data_2/" + "_".join([pde_string, mode])
    if mode == "train":
        save_name = save_name + "_" + str(num_samples)
    if suffix:
        save_name = save_name + '_' + suffix
...
def generate_data(experiment: str,
    ...
    if args.log:
        logfile = f'data_2/log/{experiment}_time{timestring}.csv'
...
if __name__ == "__main__":
    ...
    parser.add_argument('--train_samples', type=int, default=2**7,
                        help='Samples in the training dataset')
    parser.add_argument('--valid_samples', type=int, default=2 ** 0,
                        help='Samples in the validation dataset')
    parser.add_argument('--test_samples', type=int, default=2 ** 0,
